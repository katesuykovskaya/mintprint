...
//    public $formClass = 'xupload.models.XUploadForm';
    public $formClass = 'xupload.models.Attachment';
    ...
    public function init( ) {
        ...
        if( !isset( $this->path ) ) {
            $this->path = realpath( Yii::getPathOfAlias('webroot')."/uploads" );
        }
...
//            $model->{$this->fileNameAttribute} = $model->{$this->displayNameAttribute};
            ...
            $model->attachment_entity = 'pagE';
            $model->entity_id = '33';
            $model->type = $model->{$this->fileAttribute}->getType();
            ...
            if ($model->validate()) {
                ...
                $path = $this->getPath();
                $name = $model->{$this->fileAttribute}->getName();
