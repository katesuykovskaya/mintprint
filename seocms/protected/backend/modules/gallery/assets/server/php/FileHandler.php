...
//    protected $options;
    ...
    function __construct($initialize = true, $options = null, $error_messages = null) {
        if($this->setFileSession()){
            ...
            $this->options = array(
                'storage'=>'db',
                'script_url' => Yii::app()->request->hostInfo.DIRECTORY_SEPARATOR.'backend/gallery/gallery/show/',
//                'script_url' => '/backend/gallery/gallery/show/',
    ...
    /* если юзать параметры из сесссии напрямую - надо убирать true */
                ...
                'content_path' => $fileSession['files']['webUrl'].$fileSession['files']['entity'].DIRECTORY_SEPARATOR.$fileSession['files']['entity_id'].DIRECTORY_SEPARATOR,
...
//                ),
//                    'small' => array(
//                        'upload_dir' =>$fileSession['files']['filePath'].'small/',
//                        'max_width' => 300,
//                        'max_height' => 150
//                    ),
                    'thumbnail' => array(
...
//                        'upload_dir'=>$fileSession['files']['filePath'].'thumbnail/',
                        'upload_dir'=>$fileSession['files']['webUrl'].'thumbnail/',
                        'crop' => true,
                        'max_width' => 80,
                        'max_height' => 80
                    )
                ...
                foreach($result as $k=>$v){
                    ...
                    if(!empty($this->options['image_versions'])){
                        foreach($this->options['image_versions'] as $key=>$version){
                            if(!empty($key))
                                $item->{$key.'Url'} = $this->options['upload_url'].$key.DIRECTORY_SEPARATOR.$v['path'];
                            else
                                $item->url = $this->options['upload_url'].$v['path'];
                        }
                        $item->deleteUrl = $this->options['script_url'].'?file='.$v['path'];
                        $item->deleteType = $this->options['delete_type'];
        ...
        foreach($file_names as $file_name) {
            ...
            if ($success) {
                foreach($this->options['image_versions'] as $version => $options) {
                        ...
                        if (is_file($file)) {
                            unlink($file);
                            Attachment::model()->deleteAllByAttributes(array('path'=>$file));
                        }
                }
            }
