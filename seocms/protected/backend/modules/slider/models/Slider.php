...
class Slider extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
            'translation'=>array(self::HAS_MANY, 'SliderTranslate', 't_id', 'index'=>'t_language')
		);
	}
...
//            ),
    ...
    protected function afterSave() {
        if($this->isNewRecord) {
            $translate = new SliderTranslate;
            $translate->attributes = $_POST['SliderTranslate'];
            $translate->t_id = $this->id;
            $translate->t_language = Yii::app()->language;
            $translate->save(false);
        } else {
            $translate = Slider::model()->findByPk($this->id)->translation;
