...
//        $fullPath = dirname(Yii::app()->basePath) . $file;
        /* EasyImage's fullpath doesn't work correctly because of basepath, searches images in seocms/uploads instead of /uploads*/
    ...
    public function getHashedName($file,$params,$ext)
    {
        if($ext){
            $cacheFileExt = isset($params['type']) ? $params['type'] : pathinfo($file, PATHINFO_EXTENSION);
            return md5($file . serialize($params)).'.'.$cacheFileExt;
        }
