...
if(isset($_GET['subfolder']) && !empty($_GET['subfolder'])) {
    ...
    $cur_dir = !empty($subfolder) ? $upload_dir . $subfolder . '/' . $subdir : $upload_dir. '/' . $subdir;
    $cur_path = !empty($subfolder) ? $current_path . $subfolder .'/'. $subdir : $current_path.'/'. $subdir;
//    $thumbs_path = 'thumbs/' . $subfolder . '/';
    $thumbs_path = !empty($subfolder) ? $root.$upload_dir.'/thumbs/' . $subfolder . '/' : $root.$upload_dir.'/thumbs/';
...
//thumbs folder tests for existance above in init stage
...
//				$src_thumb = $thumbs_path.$subdir. $file;
                $src_thumb = $base_url . $cur_dir . 'thumbs/'.$file;
				//add in thumbs folder if not exist 
...
//				if(!file_exists($thumb_path)) create_img_gd(dirname( __FILE__ ). DIRECTORY_SEPARATOR.$current_path.$subfolder.DIRECTORY_SEPARATOR.$subdir.$file, $thumb_path, 122, 91);
                if(!file_exists($src_thumb)) create_img_gd($root.$src, $src_thumb, 122, 91);
				$is_img=true;
...
<!--					<img data-src="holder.js/122x91" alt="image" <?php //echo $show_original ? "class='original'" : "" ?> src="<?php //echo $src_thumb; ?>">-->
                    <img data-src="holder.js/122x91" alt="image" <?php echo $show_original ? "class='original'" : "" ?> src="<?php echo $cur_dir.'thumbs/'.$file; ?>">
					</div>
					</div>
					</a>	
					<div class="box">				
    ...
    <!----- lightbox div start ------->    
    <div id="previewLightbox" class="lightbox hide fade"  tabindex="-1" role="dialog" aria-hidden="true" style="z-index: 1100">
