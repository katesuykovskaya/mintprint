...
//    if (!is_writable($fileInfoArray['dirname'])) {  if ($this->debug) { die('The path is not writable. Please check your permissions.'); }else{ die(); }}
        ...
        $testPath = $_SERVER['DOCUMENT_ROOT'].$fileInfoArray['dirname'];
        if(!is_dir($testPath)){
            mkdir($testPath,0777,true);
        }

    if (!is_writable($_SERVER['DOCUMENT_ROOT'].$fileInfoArray['dirname'])) {  if ($this->debug) { die('The path is not writable. Please check your permissions.'); }else{ die(); }}
...
//          imagejpeg($this->imageResized, $savePath, $imageQuality);
            imagejpeg($this->imageResized, $testPath.'/'.$fileInfoArray['basename'], $imageQuality);
        } else { $error = 'jpg'; }
                break;
...
//          imagegif($this->imageResized, $savePath);
            imagegif($this->imageResized, $testPath.'/'.$fileInfoArray['basename']);
        } else { $error = 'gif'; }
                break;
...
//           imagepng($this->imageResized, $savePath, $invertScaleQuality);
            imagepng($this->imageResized, $testPath.'/'.$fileInfoArray['basename'], $invertScaleQuality);
        } else { $error = 'png'; }
                break;
